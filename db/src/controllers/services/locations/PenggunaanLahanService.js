...
const PenggunaanLahan = require("../../../apps/models/locations/PenggunaanLahanModel.js");
// const SetupGraphQL = require("../../../apps/database/SetupGraphQL.js");
const Repository = require("../../../apps/database/RepositoryService.js");

const { GetDataFormat, GetDataModel, GetGraphQLModel } = new PenggunaanLahan();
const { IsSet, SetRequest, SetParams, IsValidRequest, SetObject } = new Validator();

const $checkingPenggunaanLahans = async (req, repoData) => {
...
class PenggunaanLahanService {
	constructor() {
		this._Repository = new Repository(GetDataModel, "PenggunaanLahans");
	...
	async CreateData(req, res) {
		return new Promise(async (resolve, reject) => {
			...
			if (await $checkingPenggunaanLahans(req, roleDatas)) {
	...
	async UpdateDataByID(req, res) {
		return new Promise(async (resolve, reject) => {
			...
			const roleDatas = await this._Repository.GetTempDatas();
			if (!(await $checkingPenggunaanLahans({ id }, roleDatas))) {
...
module.exports = PenggunaanLahanService;
